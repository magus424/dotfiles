...
" tags added 2012 Aug 10 (vbulletin)
call s:AddITags(['if', 'else'])
...
func! HtmlIndent() "{{{
	...
	if swendtag
	    ...
	    if match(s:curline, '^\s*<else') >= 0
		" vbulletin, <else /> is a special case
		let indent -= &shiftwidth
		let s:newstate.baseindent -= &shiftwidth
	    endif
	endif
	call extend(b:indent, s:newstate, "force")
	return indent
